...
function twoDots1(context,x1,y1,x2,y2) {
    ...
    let scale = 0.1*Math.sqrt( Math.pow(y2-y1,2) + Math.pow(x2-x1,2) );
...
function twoDots3(context,x1,y1,x2,y2) {
    ...
    let beta = alpha - Math.PI*0.25;
    let scale = 0.05*Math.sqrt(2) * Math.sqrt( Math.pow(y2-y1,2) + Math.pow(x2-x1,2) );

    let a = Math.cos(beta) * scale;
    let b = Math.sin(beta) * scale;
    let c =  -Math.sin(beta) * scale;
    let d = Math.cos(beta) * scale;
    let e = x1;
    let f = y1;
    context.transform(a,b,c,d,e,f);
}
